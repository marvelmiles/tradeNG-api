...
export const updateMe = asyncHandler(async (req: Request, res: Response) => {
  ...
  return sendSuccess({ res, message: "Profile updated", data: { user: formatUser(user) } });
});

export const getStats = asyncHandler(async (req: Request, res: Response) => {
  const user_id = req.user!.id;

  const [total_listings, items_sold, review_summary, earnings_result] = await Promise.all([
    ...
    Transaction.aggregate<{ _id: null; total: number }>([
      { $match: { seller_id: new Types.ObjectId(user_id), status: "RELEASED" } },
...
export const getTrustScore = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findById(user_id).select("is_verified_seller status created_at").lean();
  if (!user) throw new AppError("Account not found", 404);

  const [review_summary, completed_as_seller, completed_total, disputes_as_seller] = await Promise.all([
    getReviewSummary(user_id),
    Transaction.countDocuments({ seller_id: user_id, status: "RELEASED" }),
  ...
  const account_age_days = Math.floor((Date.now() - user.created_at.getTime()) / (24 * 60 * 60 * 1000));
  const dispute_rate = completed_as_seller > 0 ? disputes_as_seller / completed_as_seller : 0;
  const verification_status = user.is_verified_seller ? "SELLER_VERIFIED" : "EMAIL_VERIFIED";
...
export const requestVerification = asyncHandler(async (req: Request, res: Response) => {
  const user = await User.findById(req.user!.id).select("is_verified_seller verification_requested_at").lean();
  if (!user) throw new AppError("Account not found", 404);
  if (user.is_verified_seller) throw new AppError("You are already a verified seller", 409);
  if (user.verification_requested_at) throw new AppError("Verification request already pending review", 409);

  await User.findByIdAndUpdate(req.user!.id, { verification_requested_at: new Date() });

  return sendSuccess({ res, message: "Verification request submitted. We'll review and notify you." });
});
...
export const updateNotificationSettings = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.user!.id,
    {
      ...(updates.email_general !== undefined && { "notification_settings.email_general": updates.email_general }),
      ...(updates.email_offers !== undefined && { "notification_settings.email_offers": updates.email_offers }),
      ...(updates.in_app_general !== undefined && { "notification_settings.in_app_general": updates.in_app_general }),
      ...(updates.in_app_offers !== undefined && { "notification_settings.in_app_offers": updates.in_app_offers }),
    },
    { new: true }
  )
...
export const getWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (pagination.pagination_type === "cursor") {
    const items = await WishlistItem.find({ ...where, ...buildCursorFilter(pagination.cursor) })
      .sort({ _id: -1 })
    ...
    return sendSuccess({
      res,
      data: { wishlist: items.slice(0, pagination.limit).map((w) => ({ id: w._id.toString(), listing: w.listing_id })) },
  ...
  const [items, total] = await Promise.all([
    WishlistItem.find(where).sort({ _id: -1 }).skip(skip).limit(pagination.limit).populate(populateOptions).lean(),
    WishlistItem.countDocuments(where),
  ]);

  return sendSuccess({
    res,
    data: { wishlist: items.map((w) => ({ id: w._id.toString(), listing: w.listing_id })) },
...
export const addToWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await WishlistItem.findOne({ user_id, listing_id: listingId }).lean();
...
export const removeFromWishlist = asyncHandler(async (req: Request, res: Response) => {
  const { listingId } = req.params;
  await WishlistItem.deleteOne({ user_id: req.user!.id, listing_id: listingId });
